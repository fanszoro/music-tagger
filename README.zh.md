...
- **🛡️ 纯粹私有化**：您的所有数据、播放记录和音乐文件均安全地存放在您自己的服务器上。
- **🚀 极致性能**：基于 Python FastAPI 结合 Nuitka 原生深度编译，极速的 API 响应。
- **🎨 现代化交互**：基于 Vue 3 + TailwindCSS 打造响应式单页面应用（SPA），提供丝滑的沉浸式管理体验。
- **🔌 自动化刮削**：智能接入各类数据源自动匹配并补全（支持 AcousticID、MusicBrainz、Last.fm 等扩展），杜绝手动录单。
- **🐳 极简部署**：一键式 Docker Compose 部署，原生包含数据库和队列服务，自动处理依赖。
